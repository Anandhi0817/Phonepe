...
def animate_all_amount():
    ...
    for year in map_user["Years"].unique():
        for quarter in aggregated_transaction["Quarter"].unique():
            ...
            at1 = aggregated_transaction[(aggregated_transaction["Years"]==year)&(aggregated_transaction["Quarter"]==quarter)]
...
def payment_count():
    attype= aggregated_transaction[["Transaction_type", "Transaction_count"]]
...
def payment_amount():
    attype= aggregated_transaction[["Transaction_type","Transaction_amount"]]
...
def transaction_amount_year(sel_year):
    ...
    year= int(sel_year)
    atay= aggregated_transaction[["States","Years","Transaction_amount"]]
    atay1 = aggregated_transaction.loc[(aggregated_transaction["Years"] == year)]
...
def payment_count_year(sel_year):
    year= int(sel_year)
    apc= aggregated_transaction[["Transaction_type", "Years", "Transaction_count"]]
    apc1= apc.loc[(aggregated_transaction["Years"]==year)]
...
def transaction_count_year(sel_year):
    ...
    year= int(sel_year)
    atcy= aggregated_transaction[["States", "Years", "Transaction_count"]]
    atcy1= atcy.loc[(aggregated_transaction["Years"]==year)]
...
def payment_amount_year(sel_year):
    year= int(sel_year)
    apay = aggregated_transaction[["Years", "Transaction_type", "Transaction_amount"]]
    apay1= apay.loc[(aggregated_transaction["Years"]==year)]
...
def reg_state_all_TA(sel_year,state):
    year= int(sel_year)
    mts= map_transaction[["States", "Years","Districts", "Transaction_amount"]]
    mts1= mts.loc[(map_transaction["States"]==state)&(map_transaction["Years"]==year)]
...
with col3:
        st.video("C:/Users/SHALINI/Desktop/phonepe/Phonepe graphics.mp4")
...
with col6:
    st.video("C:/Users/SHALINI/Desktop/phonepe/Phonepe graphics_2.mp4")
